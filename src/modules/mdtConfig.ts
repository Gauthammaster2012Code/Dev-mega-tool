...
export interface MdtConfig {
	...
	privacy?: { shareWithAI: "none" | "selected" | "all" };
	verification?: Array<{ provider: string; ok: boolean; reason?: string; verifiedAt: string }>;
}

const CONFIG_NAME = ".mdt-config.json";

export async function loadMdtConfig(repoRoot: string): Promise<MdtConfig> {
	const path = resolve(repoRoot, CONFIG_NAME);
	try {
		await access(path, constants.F_OK);
		const raw = await readFile(path, "utf8");
		return JSON.parse(raw) as MdtConfig;
	} catch {
		return { aiProvider: null, apiKeys: {}, preferences: {}, privacy: { shareWithAI: "selected" }, verification: [] };
	}
}

export async function saveMdtConfig(repoRoot: string, cfg: MdtConfig): Promise<void> {
	const path = resolve(repoRoot, CONFIG_NAME);
...
export async function verifyApiKey(provider: string, apiKey: string, baseURL?: string): Promise<{ ok: boolean; provider: string; reason?: string; remediation?: string }> {
	...
	try {
		...
		if (String(err?.message || "").toLowerCase().includes("unauthorized")) remediation = "Key unauthorized. Recreate key or ensure correct provider/baseURL.";
