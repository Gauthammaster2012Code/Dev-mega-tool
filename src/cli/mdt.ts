...
async function main() {
	...
	if (!cmd || cmd === "-h" || cmd === "--help") { printHelp(); return; }
	if (cmd === "interactive") {
		console.log("Interactive mode is minimal. Use 'run <tool>' for now.");
		return;
	}
	if (cmd === "run") {
		const toolName = rest[0] as any;
	...
	if (cmd === "analyze") {
		const out = await callMDTTool("analyze_codebase" as any, {});
		console.log(JSON.stringify(out, null, 2));
		return;
	}
	if (cmd === "test") {
		const withAi = rest.includes("--with-ai");
		const out = await callMDTTool(withAi ? ("run_tests_with_ai_eval" as any) : ("run_tests" as any), {});
		console.log(JSON.stringify(out, null, 2));
		return;
	}
	if (cmd === "visual") {
		...
		const out = await callMDTTool("run_visual_tests" as any, { url, name });
		console.log(JSON.stringify(out, null, 2));
		return;
	}
	if (cmd === "config") {
		...
		const out = await callMDTTool("setup_config" as any, { aiProvider: provider, apiKey: key, baseURL: base, testConnection: true });
		console.log(JSON.stringify(out, null, 2));
		return;
	}
	printHelp();
