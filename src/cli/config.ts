...
class ConfigManager {
	...
	private async updateEnvFile(config: Config): Promise<void> {
		let envContent = '';
	...
	async promptForInput(question: string, mask = false): Promise<string> {
...
async function showCurrentConfig() {
	...
	for (const { key, name, provider } of providers) {
		const hasKey = !!(config[key as keyof Config] || process.env[key]);
...
async function configureOpenAI(configManager: ConfigManager, config: Config) {
	console.log('\n🔑 Configure OpenAI');
...
async function configureClaude(configManager: ConfigManager, config: Config) {
...
async function configureGemini(configManager: ConfigManager, config: Config) {
...
async function configureQwen(configManager: ConfigManager, config: Config) {
