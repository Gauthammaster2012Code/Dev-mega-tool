...
import { ToolCaller, type ToolResult } from "../modules/toolCalling.js";
...
class EnhancedChatInterface {
	...
	async selectProvider() {
		...
		const availableProviders: { name: string; key: string; provider: () => AIProvider | null }[] = [
			{
	...
	private showHistory() {
		...
		this.conversation.forEach((msg, index) => {
			...
			let role = '🤖 AI';
			if (msg.role === 'user') role = '👤 You';
			else if (msg.role === 'system') role = '⚙️  System';
	...
	private async generateEnhancedResponse(userMessage: string): Promise<string> {
		if (!this.provider || !this.toolsEnabled) {
		...
		const shouldUseTool = toolKeywords.some(keyword => 
	...
	private async handleToolSuggestedMessage(userMessage: string): Promise<string> {
		...
		if (message.includes('current branch') || message.includes('git status')) {
			...
			if (statusResult.success) {
				...
				statusResult.data.changes.forEach((change: any) => {
