...
export async function writeMcpKey(repoRoot, key) {
...
export async function readMcpKey(repoRoot) {
    try {
        const path = resolve(repoRoot, KEY_FILE);
...
export async function ensureMcpKey(repoRoot) {
...
export async function verifyMcpKey(repoRoot, provided) {
    if (!provided)
        return false;
    const fileKey = await readMcpKey(repoRoot);
    return !!fileKey && provided === fileKey;
}
//# sourceMappingURL=mcpKey.js.map